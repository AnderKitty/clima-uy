...
const TTL = { estado: 300e3, pronostico: 300e3, catalogo: 86400e3, home: 600e3 };
...
async function traer(clave, { comoTexto = false } = {}) {
  ...
  const promesa = (async () => {
    ...
    if (!r.ok) throw new Error(`${clave}: HTTP ${r.status}`);
    const datos = comoTexto ? await r.text() : await r.json();
...
/** `categoria` es un objeto {riesgoViento, riesgoLluvia, riesgoTormenta}
 *  donde solo los riesgos vigentes traen valor. Lo pasamos a lista de nombres. */
function riesgosDe(categoria) {
  if (!categoria || typeof categoria !== 'object') return [];
  return Object.entries(categoria)
    .filter(([, v]) => v)
    .map(([k]) => k.replace(/^riesgo/, '').toLowerCase());
}

/**
 * El campo `zonas` viene como un bloque corrido donde los departamentos se
 * pegan sin separador ("…y Topador.Cerro Largo : Arévalo…"). Lo partimos en
 * {departamento, localidades} para poder mostrarlo prolijo.
 */
...
function extraerAvisos(html) {
  ...
  for (const nombre of ['aviso', 'alerta']) {
    const m = html.match(new RegExp(`var\\s+${nombre}\\s*=\\s*(\\{[\\s\\S]*?\\});\\s*(?=var\\s|<\\/script>)`));
    if (!m) continue;
    let d;
    ...
    const items = d.advertencias.map((a) => ({
      ...
      zonas: partirZonas(a.zonas),
      comienzo: a.comienzo ?? null,
      finalizacion: a.finalizacion ?? null,
      riesgos: riesgosDe(a.categoria ?? d.mapaMergeCategoria),
    }));
...
export async function avisos() {
  const h = await traer('home', { comoTexto: true });
  return { ...extraerAvisos(h.datos), rancio: h.rancio };
}
